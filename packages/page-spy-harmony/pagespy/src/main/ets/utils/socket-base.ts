...
import type { SpyMessage, SpySocket, SpyBase } from '../types';
...
import { InteractiveType } from '../types/lib/message-type';
...
export abstract class SocketStoreBase {
  ...
  public init(url: string) {
    try {
      ...
      this.socketWrapper?.onMessage((data: string) => {
  ...
  protected handleMessage(data: string) {
    ...
    const result = JSON.parse(data) as SpySocket.Event;
    const { type } = result;
    switch (type) {
      case CONNECT:
  ...
  private static handleResolveAtom(
    ...
    if (type === DEBUG_MESSAGE_TYPE.ATOM_DETAIL) {
      ...
      const msg = makeMessage(
        `atom-detail-${data}` as InteractiveType,
        atomData,
        false,
      );
      reply(msg);
  ...
  private static handleAtomPropertyGetter(
    ...
    if (type === DEBUG_MESSAGE_TYPE.ATOM_GETTER) {
      ...
      /* c8 ignore stop */
      const msg = makeMessage(
        `atom-getter-${id}` as InteractiveType,
        atom.transformToAtom(value),
      );
      reply(msg);
    }
  }
